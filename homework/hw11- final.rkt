...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname pipe-fantasy-final-wip) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; Please run the following code below to play the game: 
;; (pipe-fantasy THE-GAME)
...
;; pipe->image: Pipe Integer Integer Boolean Boolean-> Image
...
(define (pipe->image pipe tile-side-length pipe-width filled? direction crossed?)
  (local [;; color : Boolean -> String
          ...
          ;; draw-cross-pipe : String -> Image
          ;; Purpose: Draws the cross pipe
          (define (draw-cross-pipe color)
            (cond
              [crossed? (overlay (overlay/align
                                  "middle" "middle"
                                  (rectangle tile-side-length pipe-width "solid" color)
                                  (rectangle pipe-width tile-side-length "solid" color))
                                 (square tile-side-length "outline" "black"))]
              [(or (string=? direction LEFT)
...
(define-struct pc [pipe row col fill? direction crossed?])
;; A  PC is a (make-pipe-coord Pipe Integer Integer Boolean Flow Boolean)
...
;; - direction - is the direction of the flow of the pipe
;; - crossed? - determines whether or not a cross pipe has been flowed
;; through twice
;; Examples:
(define PC-1 (make-pc PIPE-TL 5 3 #false "nil" #false))
(define PC-2 (make-pc PIPE-BR 2 3 #false "nil" #false))
(define PC-3 (make-pc PIPE-LR 9 9 #false "nil" #false))
(define SPC-1 (make-pc OP-RIGHT 1 2 #true "nil" #false))
(define SPC-2 (make-pc OP-LEFT 3 2 #true "nil" #false))
(define SPC-3 (make-pc OP-TOP 5 1 #true "nil" #false))
;; Template:
;; pc-templ : PC -> ?
(define (pc-templ pc)
...
(define-struct grid [size pc replaced])
...
;; - replaced - is the number of pipes that have been replaced before
;; Examples:
(define GRID-1 (make-grid 5 (list PC-1 PC-2) 0))
(define GRID-2 (make-grid 7 (list PC-2) 0))
(define GRID-3 (make-grid 10 (list PC-1 PC-2 PC-3) 0))
(define GRID-4 (make-grid 3 (list (make-pc PIPE-BTLR 3 3 #false "nil" #false)) 0))
;; Template: 
;; grid-templ : Grid -> ?
(define (grid-templ g)
  (...(grid-size g)...
...
(define STARTING-GRID (make-grid 7 empty 0))
...
;; Testcases:
(check-expect (place-pipe GRID-1 PIPE-BL 3 2 #false "nil" #false)
              (make-grid (grid-size GRID-1)
                         (cons (make-pc PIPE-BL 3 2 #false "nil" #false)
                               (cons PC-1 (cons PC-2 empty))) 0))
(check-expect (place-pipe GRID-2 PIPE-TL 2 3 #false "nil" #false)
             (make-grid (grid-size GRID-2)
                        (cons (make-pc PIPE-TL 2 3 #false "nil" #false) empty) 1))
(check-expect (place-pipe GRID-3 PIPE-BTLR 6 3 #false "nil" #false)
             (make-grid (grid-size GRID-3)
                        (cons (make-pc PIPE-BTLR 6 3 #false "nil" #false)
                              (cons PC-1 (cons PC-2 (cons PC-3 empty)))) 0))
(define (place-pipe grid pipe row col fill? direction cross?)
  (make-grid (grid-size grid)
             (remove-extra (make-pc pipe row col fill? direction cross?) (grid-pc grid))
             (if (boolean? (pipe-at grid row col ))
                 (grid-replaced grid)
                 (+ 1 (grid-replaced grid)))))

;; remove-extra : PC [List-of PC] -> [List-of PC]
;; Purpose: Removes the old pipe if a new pipe is placed at the same coordinates
;; Testcases:
(check-expect (remove-extra (make-pc PIPE-BR 2 3 #false "nil" #false) (cons PC-2 empty))
              (cons (make-pc PIPE-BR 2 3 #false "nil" #false) empty))
(check-expect (remove-extra (make-pc PIPE-TL 1 1 #false "nil" #false) (cons PC-1 (cons PC-2 empty)))
              (cons (make-pc PIPE-TL 1 1 #false "nil" #false) (cons PC-1 (cons PC-2 empty))))
...
(check-expect (create-row 3 3 80 40 GRID-4)
              (beside (square 80 "outline" "black")
                      (square 80 "outline" "black")
                      (pipe->image PIPE-BTLR 80 40 #false "nil" #false)))
  ...
  (cond
    ...
    [(and (not (= c 0)) (= 0 (- c 1)))
     (pipe->image (pc-pipe (pipe-at grid r (+ (- (grid-size grid) c) 1))) l pw
                  ...
                  (pc-direction (pipe-at grid r (+ (- (grid-size grid) c) 1)))
                  (pc-crossed? (pipe-at grid r (+ (- (grid-size grid) c) 1))))]
    [(not (= c 0))
...
;; Examples:
(define GF-1 (make-gooflow (list (make-pc PIPE-TL 3 2 #true "LEFT" #false)) LEFT))
...
;; Testcases:
(check-expect (opening? LEFT (make-pc PIPE-BTLR 3 4 #false "nil" #false)) #true)
(check-expect (opening? RIGHT (make-pc PIPE-BTLR 3 4 #false "nil" #false)) #true)
(check-expect (opening? UP (make-pc PIPE-BTLR 3 4 #false "nil" #false)) #true)
(check-expect (opening? DOWN (make-pc PIPE-BTLR 3 4 #false "nil" #false)) #true)
(check-expect (opening? UP (make-pc PIPE-LR 3 4 #false "nil" #false)) #false)
(define (opening? f p)
  (cond
...
;; grid-goo-propagate : GooFlow Grid -> GooFlow  [1 2 right
...
(check-expect (grid-goo-propagate (make-gooflow (list SPC-1) RIGHT)
                                  (make-grid 4
                                             (list SPC-1
                                                   (make-pc PIPE-BTLR 1 3 #false "nil" #false)) 0))
              (make-gooflow (list (make-pc PIPE-BTLR 1 3 #true "Right" #false) SPC-1) RIGHT))
(check-expect (grid-goo-propagate (make-gooflow (list SPC-1) RIGHT)
                                  (make-grid 4 (list SPC-1) 0))
              (make-gooflow (list SPC-1) RIGHT))
(define (grid-goo-propagate gf g)
  (local [;; cross-pipe? : Pipe -> Boolean
          ;; Purpose: Determines whether or not a pipe is a cross pipe
          (define (cross-pipe? pipe)
            (and (pipe-top pipe)
                 (pipe-bot pipe)
                 (pipe-left pipe)
                 (pipe-right pipe)))]
  ...
  (if (and (not (boolean? (pipe-at g (first (move gf)) (second (move gf)))))
      ...
      (if (and (cross-pipe? (pc-pipe (pipe-at g (first (move gf)) (second (move gf)))))
               (in-flow? (first (move gf)) (second (move gf)) (gooflow-path gf)))
          (make-gooflow (cons
                         (make-pc
...
(define-struct game-state [grid st-pipe gf in-pipes tl pw tick])
...
(define GS-1 (make-game-state
              (make-grid 7 (list SPC-1) 0)
              SPC-1 
              (make-gooflow (list SPC-1) RIGHT)
              (append ALL-PIPES (list PIPE-TR)) 80 40 140))
(define GS-2 (make-game-state
              (make-grid 5 (list SPC-2) 0)
              SPC-2 
              (make-gooflow (list SPC-2) LEFT)
              (list PIPE-TL PIPE-BTLR)
              80 40 140))
(define GS-3 (make-game-state
              (make-grid 8 (list SPC-3) 0)
              SPC-3
...
(define (gamestate-init size r c fl inc-pipes)
  (make-game-state
   (make-grid size (list (make-pc (pipe-on-op fl) r c #true "nil" #false)) 0)
   (make-pc (pipe-on-op fl) r c #true "nil" #false)
   (make-gooflow (list (make-pc (pipe-on-op fl) r c #true "nil" #false)) fl)
   inc-pipes
   80 40 140))
...
(check-expect (place-pipe-on-click GS-1 100 100 "button-down")
              (make-game-state
               (place-pipe
                ...
                (helper-click 100 (game-state-tl GS-1))
                #false
                (gooflow-flo (game-state-gf GS-1))
                #false)
               SPC-1
               (game-state-gf GS-1)
               ...
               (game-state-tl GS-1)
               (game-state-pw GS-1)
               (game-state-tick GS-1)))
(check-expect (place-pipe-on-click
               (make-game-state
                (make-grid 5 (list (make-pc PIPE-BTLR 1 3 #false "nil" #false) SPC-1) 0)
                SPC-1
                (make-gooflow (list SPC-1) RIGHT)
                empty
                80 40 13)
...
(define (place-pipe-on-click state x y event)
  (if (and
       ...
       (not (in-flow? (helper-click y (game-state-tl state))
                      (helper-click x (game-state-tl state))
                      (gooflow-path (game-state-gf state))))
        ...
        [(cons? (game-state-in-pipes state))
         (make-game-state
          (place-pipe
           ...
           (helper-click x (game-state-tl state))
           #false
           (gooflow-flo (game-state-gf state))
           #false)
...
;; in-flow? : Integer Integer [List-of PC] -> Boolean
;; Purpose: Checks if the current placement click is a part of the
;; path of gooflow
;; Testcases:
(check-expect (in-flow? 5 1 (list PC-1 PC-2 PC-3)) #false)
(define (in-flow? r c lis)
  (ormap (lambda (x) (and (= r (pc-row x)) (= c (pc-col x)))) lis))

;; the-goo : GameState -> GameState
;; Purpose: Propogates the goo based on time (ticks)
;; Testcases:
(check-expect (the-goo GS-1)
              (make-game-state
               (make-grid (grid-size (game-state-grid GS-1))
                          (grid-pc (game-state-grid GS-1))
                          (grid-replaced (game-state-grid GS-1)))
               (game-state-st-pipe GS-1)
               (game-state-gf GS-1)
               (game-state-in-pipes GS-1)
               (game-state-tl GS-1)
               (game-state-pw GS-1)
               (- (game-state-tick GS-1) 1)))
(check-expect (the-goo (make-game-state
                        (make-grid (grid-size (game-state-grid GS-1))
                                   (grid-pc (game-state-grid GS-1))
                                   (grid-replaced (game-state-grid GS-1)))
                        (game-state-st-pipe GS-1)
...
(define (the-goo state)
  (local [;; goo-helper : GameState -> [List-of PC]
   ...
   (if (= (game-state-tick state) 0)
       (make-game-state
        (make-grid (grid-size (game-state-grid state))
                   (goo-helper state)
                   (grid-replaced (game-state-grid state)))
        (game-state-st-pipe state)
        (grid-goo-propagate
         (game-state-gf state)
         (game-state-grid state))
       ...
       (make-game-state
        ...
        (game-state-tl state)
        (game-state-pw state)
        (- (game-state-tick state) 1)))))

;; draw-state : GameState -> Image
;; Purpose: Draws out the current game board
;; Testcases:
(check-expect (draw-state GS-1)
              (above (grid->image (make-grid 7 (list (game-state-st-pipe GS-1)) 0) 80 40)
                     (square 20 "solid" "transparent")
                     (beside (score->image (get-score GS-1))
...
(define (draw-state state)
  (above (grid->image (game-state-grid state)
               ...
               (game-state-pw state))
         (square 20 "solid" "transparent")
         (beside (score->image (get-score state))
                 (dis-inc-pipes
                  3
                  (game-state-tl state)
                  (game-state-pw state)
...
;; get-score : GameState Integer -> Integer
;; Purpose: Calculates the score base on the number length of the path in goo
;; Testcases:
(check-expect (get-score GS-1) 0)
(check-expect (get-score (make-game-state
                          ...
                          0)) -50)
(define (get-score gs)
  (* 50 (- (- (length (gooflow-path (game-state-gf gs))) 1)
           (grid-replaced (game-state-grid gs)))))

;; score->image : Integer -> Image
;; Purpose: Turns the score into an image that can be put on the game board
;; Testcases:
(check-expect (score->image 20) (text "Score: 20  " 30 "black"))
(check-expect (score->image 50) (text "Score: 50  " 30 "black"))
(define (score->image score)
  (text (string-append "Score: " (number->string score) "  ") 30 "black"))
...
(check-expect (dis-inc-pipes 3 80 40 (list PIPE-BR PIPE-TL PIPE-TL PIPE-BR))
              (beside (pipe->image PIPE-BR 80 40 #false "nil" #false)
                      (pipe->image PIPE-TL 80 40 #false "nil" #false)
                      (pipe->image PIPE-TL 80 40 #false "nil" #false)))
...
(define (dis-inc-pipes n tl pw lis)
  (cond
    ...
    [(and (cons? lis) (= 0 (- n 1)))
     (pipe->image (first lis) tl pw #false "nil" #false)]
    [(and (cons? lis) (not (= 0 (- n 1))))
     (beside (pipe->image (first lis) tl pw #false "nil" #false)
...
;; Creating an in-progress game (for grading purposes)
(define GAME-PIPES-GRID (list
                         (make-pc PIPE-TL 3 5 #false "nil" #false)
                         (make-pc PIPE-LR 3 4 #false "nil" #false)
                         (make-pc PIPE-BR 3 2 #false "nil" #false)
                         (make-pc PIPE-BTLR 4 2 #false "nil" #false)
                         (make-pc PIPE-TR 5 2 #false "nil" #false)
                         (make-pc PIPE-TL 5 3 #false "nil" #false)
                         (make-pc PIPE-TB 4 3 #false "nil" #false)
                         (make-pc PIPE-BTLR 3 3 #false "nil" #false)
                         (make-pc PIPE-BL 2 3 #false "nil" #false)
                         (make-pc OP-RIGHT 2 2 #true "nil" #false)))
(define GAME-IN-PIPES (list PIPE-BR PIPE-TB PIPE-LR PIPE-BTLR PIPE-TL))
(define THE-GAME (make-game-state (make-grid 7 GAME-PIPES-GRID 0)
                                  (make-pc OP-RIGHT 2 2 #true "nil" #false)
                                  (make-gooflow
                                   (list (make-pc OP-RIGHT 2 2 #true "nil" #false))
                                   RIGHT)
                                  GAME-IN-PIPES
                                  80 40 140))
